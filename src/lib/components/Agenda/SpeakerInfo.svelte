...
{#if speaker.linkedin}
  ...
  >
    <!-- Avatar with Link -->
    <div
      class="w-13 tablet:w-12 h-13 tablet:h-12 desktop-xl:h-20 desktop-xl:w-20 shrink-0"
    >
      <a
        ...
        class="block w-full h-full relative group/avatar transition-transform duration-300 ease-out hover:scale-105 {isPressed
          ? 'scale-95'
          : ''}"
      >
        <div class="w-full h-full rounded-full overflow-hidden relative">
          ...
          <!-- Dark Overlay -->
          <div
            class="absolute inset-0 transition-all duration-300 ease-out opacity-0 group-hover/avatar:opacity-100 {isPressed
              ? 'bg-black/80'
              : 'bg-black/48'}"
          ></div>
          ...
          >
            <img
              ...
              alt="LinkedIn"
              class="w-6 h-6 tablet:w-7 tablet:h-7 desktop:w-4 desktop:h-4 desktop-xl:w-6 desktop-xl:h-6"
            />
          </div>
        </div>
      </a>
